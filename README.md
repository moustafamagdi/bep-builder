...
## Phase 4 — discussion, comparison and activity

Discussion & history is an unborrowed review workspace. It compares the current draft or any two frozen snapshots without modifying them. Comparisons match unique row IDs/codes when available, otherwise explicit row positions, and report additions, removals, changed values and identified row ordering. Private approval content snapshots are excluded from display.

Private section-linked comments live outside project JSON. Owners/editors can post, resolve and reopen; viewers can read. Comment bodies are retained rather than edited/deleted. Posting and resolution do not invalidate plan approvals or automatically block release. A Refresh action reloads discussion and activity. The discussion shows the latest 500 comments; activity loads in pages of 50.

Apply `supabase/review_activity.sql` once before deploying. Both tables have RLS and SELECT-only client grants. Membership-aware project SELECT policies scope reads. The authenticated comment RPC verifies current owner/editor membership and supplies author identity and timestamps from the server. Its SECURITY DEFINER capability is intentional for controlled writes and audit insertion; direct table writes remain denied. Project audit insertion is a non-callable trigger. Audit records identify actor, cloud version and changed fields/schedules on server project saves, plus comment actions. This is not a complete log of invitation/storage operations or historical actions before installation. No backfilled history is invented. Project deletion cascades to discussion/activity. Public shares and project JSON backups do not include these private server records.

`supabase/tests/review_activity.sql` runs isolated synthetic role tests in a rolled-back transaction, including owner/editor writes, viewer reads, outsider/anonymous denial and audit integrity. The existing section borrowing/save behavior remains unchanged. Supabase RLS reference: https://supabase.com/docs/guides/database/postgres/row-level-security
