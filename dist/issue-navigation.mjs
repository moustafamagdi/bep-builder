...
const listViews={compliance:'information',detailedResponsibilities:'organization',mobilization:'organization',risks:'coordination',approvals:'review',changeLog:'review',parties:'organization',team:'organization',responsibilities:'organization',references:'information',uses:'information',software:'information',exchanges:'information',models:'technical',namingFields:'technical',loin:'technical',milestones:'coordination',deliverables:'coordination',clashes:'coordination',meetings:'coordination',qaChecks:'coordination',assetRequirements:'coordination',decisions:'files',appendices:'files'};

export function issueTarget(code,project){
  if(code.startsWith('row:')){const [,key,row]=code.split(':');return {view:listViews[key]||'review',selector:`[data-list="${key}"][data-row="${Number(row)}"]`};}
  ...
  if(listViews[key]){
    let row=null;if(code==='open-decisions')row=project.lists.decisions.findIndex(item=>['Open','Proposed'].includes(item.status));if(code==='missing-appendices')row=project.lists.appendices.findIndex(item=>['Removed','Not received','Missing'].includes(item.status));
