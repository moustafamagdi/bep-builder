...
function sheetXml(key,rows){const schema=listSchemas[key],all=[schema.columns.map(c=>c[1]),...rows.map(row=>schema.columns.map(c=>row[c[0]]??''))];return `<?xml version="1.0" encoding="UTF-8" standalone="yes"?><worksheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main"><sheetViews><sheetView workbookViewId="0"/></sheetViews><sheetFormatPr defaultRowHeight="15"/><sheetData>${all.map((row,r)=>`<row r="${r+1}">${row.map((value,c)=>`<c r="${cellRef(c)}${r+1}" t="inlineStr"${r===0?' s="1"':''}><is><t xml:space="preserve">${xmlEscape(value)}</t></is></c>`).join('')}</row>`).join('')}</sheetData><autoFilter ref="A1:${cellRef(schema.columns.length-1)}${Math.max(1,all.length)}"/><cols>${schema.columns.map((_,i)=>`<col min="${i+1}" max="${i+1}" width="22" customWidth="1"/>`).join('')}</cols></worksheet>`;}
