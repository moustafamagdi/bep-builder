...
function listCell(key,rowIndex,row,[column,,type='text',options=[]]){const value=row[column]||(column==='gate'?gateFor(key,row):'');if(type==='select')return `<select data-list="${key}" data-row="${rowIndex}" data-col="${column}">${(!options.includes(value)?[value,...options]:options).map(option=>`<option ${value===option?'selected':''}>${esc(option)}</option>`).join('')}</select>`;return `<input data-list="${key}" data-row="${rowIndex}" data-col="${column}" type="${type==='readonly'?'text':type}" ${type==='readonly'?'readonly':''} value="${esc(value)}">`;}
...
$('#new-project-form').addEventListener('submit',async event=>{event.preventDefault();const submitter=event.submitter;if(submitter?.value!=='create'){$('#project-dialog').close('cancel');return;}const data=new FormData(event.currentTarget),seed={},contractor=String(data.get('contractor')).trim();for(const [input,field] of [['name','projectName'],['code','projectCode']]){const value=String(data.get(input)||'').trim();if(value)seed[field]=value;}if(contractor)seed.contractor=contractor;const p=createPresetProject(String(data.get('startingPoint')||'default'),seed);workspace.projects.push(p);saveWorkspace(workspace);if(session&&!await syncProject(p,{quiet:true})){workspace.projects=workspace.projects.filter(item=>item.id!==p.id);saveWorkspace(workspace);notify('The project could not be created in the cloud.');return;}$('#project-dialog').close('create');openProject(p.id);});
