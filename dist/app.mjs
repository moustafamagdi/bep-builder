import {governanceSchemas,bindApproval,approvalValid,deliverySummary} from './governance.mjs';
...
function renderReadiness(){if(!active)return;const r=reviewProject(active);$('#readiness-value').textContent=`${r.score}%`;$('#readiness-bar').style.width=`${r.score}%`;$('#readiness-label').textContent=r.ready?'Data and approval evidence recorded':`${r.critical} data gaps · ${r.warnings} warning${r.warnings===1?'':'s'}`;$('#current-project-name').textContent=active.fields.projectName||'Untitled project';}
